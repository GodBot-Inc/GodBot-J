...
class UrlBuilder {
    ...
    class YT {
        ...
        fun getWatch() = apply {
            this.url = "https://youtube.com/watch?v=idHere"
        }

        fun getSearch() = apply {
            this.url = "https://youtube.googleapis.com/youtube/v3/search?part=sinppet" +
                    "&maxResults=5" +
                    "&q=searchHere" +
                    "&videoCategoryId=10" +
                    "&key=apiKeyHere"
        ...
        fun getApiKey(): String = Dotenv.load()["YT_API_KEY"]

        fun buildString(): String {
            var result = url

            if (url.contains("q"))
               result = result.replace("searchHere", this.search)
